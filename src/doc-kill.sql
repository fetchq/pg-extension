DROP FUNCTION IF EXISTS fetchq_catalog.fetchq_doc_kill(CHARACTER VARYING, CHARACTER VARYING);
CREATE OR REPLACE FUNCTION fetchq_catalog.fetchq_doc_kill(
	PAR_queue VARCHAR,
	PAR_subject VARCHAR,
...
BEGIN
	...
	VAR_q = VAR_q || 'UPDATE fetchq_catalog.%s__documents AS lc SET ';
    VAR_q = VAR_q || 'status = -1,';
    VAR_q = VAR_q || 'attempts = 0,';
    VAR_q = VAR_q || 'iterations = lc.iterations + 1,';
    VAR_q = VAR_q || 'last_iteration = NOW()';
	VAR_q = VAR_q || 'WHERE subject IN( SELECT subject FROM fetchq_catalog.%s__documents WHERE subject = ''%s'' AND status = 2 LIMIT 1 ) RETURNING version) ';
...
DROP FUNCTION IF EXISTS fetchq_catalog.fetchq_doc_kill(CHARACTER VARYING, CHARACTER VARYING, JSONB);
CREATE OR REPLACE FUNCTION fetchq_catalog.fetchq_doc_kill(
	PAR_queue VARCHAR,
	PAR_subject VARCHAR,
	PAR_payload JSONB,
